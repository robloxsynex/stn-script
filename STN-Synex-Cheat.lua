...
-- ============================================================
--  INSTANT INTERACT (отключает задержку у ProximityPrompt)
-- ============================================================
local instantInteractActive = false
local savedHoldDurations = {} -- храним оригинальные значения HoldDuration
...
local function enableInstantInteract()
    ...
    mt.__index = newcclosure(function(t, k)
        if not checkcaller() and k == "HoldDuration" and t:IsA("ProximityPrompt") then
            return 0
        end
        return oldIndex(t, k)
    end)
    ...
    -- Сбрасываем существующие промпты
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("ProximityPrompt") then
            if not savedHoldDurations[v] then
                savedHoldDurations[v] = v.HoldDuration
            end
            v.HoldDuration = 0
        end
    end
...
local function disableInstantInteract()
    ...
    local oldIndex = mt.__index
    setreadonly(mt, false)
    -- Просто возвращаем старый метаметод (он уже сохранён в старой переменной)
    mt.__index = oldIndex
    setreadonly(mt, true)
    
    -- Восстанавливаем сохранённые значения
    for prompt, orig in pairs(savedHoldDurations) do
