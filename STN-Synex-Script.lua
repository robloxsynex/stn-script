--[[
    STN GUI - Mobile Edition (by Synex)
]]
...
Menu.Size = UDim2.new(0, 210, 0, 340)
...
-- ============================================================
--  ANTI-CHEAT BYPASS (расширенный: скорость + CFrame)
-- ============================================================
local bypassEnabled = false
local realWalkSpeed = 16  -- реальная скорость, которую видит игрок
local fakeWalkSpeed = 16  -- скорость, которую видит античит
...
local function enableBypass()
    ...
    -- Перехват записи в Humanoid (WalkSpeed, JumpPower)
    ...
    mt.__newindex = newcclosure(function(t, k, v)
        if not checkcaller() and t:IsA("Humanoid") then
            if k == "WalkSpeed" then
                -- Сохраняем реальную скорость, но античиту отдаём заниженную
                realWalkSpeed = v
                return oldNewIndex(t, k, fakeWalkSpeed)
            elseif k == "JumpPower" then
                return nil  -- блокируем изменение силы прыжка античитом
            end
        end
        return oldNewIndex(t, k, v)
    end)
    
    -- Перехват чтения WalkSpeed (чтобы античит видел маленькую скорость)
    ...
    -- Блокируем проверки CFrame (телепортации)
    local oldCFrameIndex
    oldCFrameIndex = hookmetamethod(game, "__index", function(self, k)
        if not checkcaller() and self:IsA("BasePart") and k == "CFrame" then
            -- Если античит читает позицию, возвращаем старую, не обновлённую
            -- (это сложно, проще блокировать события)
        end
        return oldCFrameIndex(self, k)
    end)
    
    -- Отключаем скрипты античита
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("LocalScript") then
            local name = v.Name:lower()
            if name:find("anticheat") or name:find("adoni") or name:find("checker") or name:find("speed") then
                v.Disabled = true
            end
        end
    end
    
    print("Anti-Cheat Bypass ON (WalkSpeed/Teleport защищены)")
end

local function disableBypass()
    bypassEnabled = false
    -- Восстанавливаем метатаблицы сложно, проще перезапустить скрипт? Но не будем усложнять.
    print("Anti-Cheat Bypass OFF (рекомендуется перезапустить скрипт)")
end

-- ============================================================
--  ESP: ТЕКСТОВЫЕ НАДПИСИ ДЛЯ ОБЪЕКТОВ (без изменений)
-- ============================================================
local activeTags = {}
...
local function updateBlacklightCaseTag()
    if espObjectStates.blacklightCase then
        for _, v in pairs(Workspace:GetDescendants()) do
            local name = v.Name
            if name == "BlacklightCase" or name == "BLACKLIGHTCase" or name == "Blacklight Case" then
                addObjectTag(v, "BLACKLIGHT CASE", Color3.fromRGB(180, 60, 255))
            end
        end
    else
        removeTagsByCondition(function(tag)
            return tag.Adornee and (tag.Adornee.Name == "BlacklightCase" or tag.Adornee.Name == "BLACKLIGHTCase" or tag.Adornee.Name == "Blacklight Case")
        end)
    end
end
...
-- ============================================================
--  ФУНКЦИИ ДЛЯ КНОПОК (ползунки, toggle)
-- ============================================================
    ...
    itemAuraThread = coroutine.create(function()
        while itemAuraActive do
            ...
            pcall(function()
                ...
                for _, v in pairs(Workspace.Misc:GetDescendants()) do
                    if v.Name == "MurchCase" or v.Name == "AILACase" or v.Name == "BlacklightCase" or v.Name == "BLACKLIGHTCase" then
...
-- ============================================================
--  INSTANT INTERACT (без изменений)
-- ============================================================
...
local function startAirJump()
    ...
    jumpRequestConnection = UserInputService.JumpRequest:Connect(function()
        ...
        if currentChar and currentHum then
    ...
    airJumpConnection = RunService.Heartbeat:Connect(function()
        ...
        if currentHum and hrp then
            if currentHum.FloorMaterial == Enum.Material.Air and not currentHum.Sit then
                local p = getAirJumpPlatform()
                local platformY = hrp.Position.Y - 3.5
                p.CFrame = CFrame.new(hrp.Position.X, platformY, hrp.Position.Z)
                p.CanCollide = true
            else
...
makeToggleButton("Relic Aura", function(state)
    ...
    if state then startRelicAura() else
...
makeToggleButton("Item Aura", function(state)
    ...
    if state then startItemAura() else
...
-- Walk Speed (с защитой от отката через Bypass)
    ...
    walkSpeedConnection = RunService.Heartbeat:Connect(function()
        if walkSpeedActive then
            ...
            if hum then
                ...
                -- Если включён Bypass, то fakeWalkSpeed останется маленькой
...
makeToggleButton("AirJump", function(state)
    ...
    if state then startAirJump() else stopAirJump() end
end)
...
-- Item Farm (увеличенная задержка 2.5 сек)
...
local function isSurvivor()
    ...
    for _, item in pairs(backpack:GetChildren()) do
        ...
        if name:find("flashlight") or name:find("medkit") or name:find("bandage") then
            return true
        end
    end
    return false
                ...
                pcall(function()
                    ...
                    for _, target in pairs(targets) do
                        ...
                        if hrp then
                            ...
                            task.wait(2.5)  -- увеличенная задержка
